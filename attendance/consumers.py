...
class SmartScreenConsumer(AsyncWebsocketConsumer):
    ...
    def get_initial_students(self):
        # استخدام .values() لجلب البيانات المطلوبة فقط مباشرة من DB دون بناء كائنات Django كاملة
...
class SmartScreenConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        if not token:
            await self.send_error("MISSING_TOKEN", "التوكن مطلوب للاتصال")
            await self.close(code=4003)
            return
    ...
    async def disconnect(self, close_code):
        ...
        print(f"[SmartScreen] disconnected with code: {close_code}")

    @database_sync_to_async
    def verify_screen(self, token):
        try:
            # استخدام select_related لتقليل استعلامات القاعدة عند التحقق
            screen = SmartScreen.objects.select_related("school_class").get(
                screen_token=token,
                is_active=True
            )

            if not screen.school_class or screen.school_class.school_id != screen.school_id:
                return None
